function A = convnfft_c(A, B, dims)
% Circular convnfft, modiefied by Yang Qi 08/04/1015
...
if GPU % GPU/Jacket FFT
    ...
    %>>>>>>>>
    A=repmat(A,[2 2]); %extend the periodic bounary; added by Qi
    B0=zeros(size(A));
    indxi = mod( (1:size(B,1))-1-floor(size(B,1)/2) ,size(A,1))+1;
    indxj = mod( (1:size(B,2))-1-floor(size(B,2)/2) ,size(A,2))+1;
    B0(indxi,indxj)=B;
    B=B0;
    %<<<<<<<<
...
if ABreal
    ...
    %A = real(A(subs{:}));
    %indx= floor(m/8)+1:(floor(m/8)+m/2);
    indx = 1:(m/2);
    A=real(A(   1:size(A,1)/2   , 1:size(A,2)/2    ));
